...
def test_summary_postgres_and_missing_optional_components(super_admin_api):
 r=super_admin_api.get(f'{BASE}/api/system-health',timeout=10);assert r.status_code==200,r.text;b=r.json();by={x['component']:x for x in b['components']}
...
def test_kiosk_online_degraded_offline_and_fleet_counts(db,super_admin_api):
 ...
 r=super_admin_api.get(f'{BASE}/api/system-health/kiosks',timeout=10).json();items=[x for x in r['items'] if x['device_uuid'].startswith('V69-')];assert {x['normalized_status'] for x in items}=={'ONLINE','DEGRADED','OFFLINE'}
def test_job_failed_missed_disabled(db,super_admin_api):
 ...
 b=super_admin_api.get(f'{BASE}/api/system-health',timeout=10).json();jobs=next(x for x in b['components'] if x['component']=='JOBS');states={x['normalized_status'] for x in jobs['details']['items'] if x['job_name'] in ('v69-fail','v69-missed','v69-disabled')};assert states=={'FAILED','MISSED','DISABLED'} and jobs['status']=='DEGRADED'

def test_job_never_run_is_its_own_status_not_healthy(db,super_admin_api):
 ...
 b=super_admin_api.get(f'{BASE}/api/system-health',timeout=10).json();jobs=next(x for x in b['components'] if x['component']=='JOBS')
...
def test_error_fingerprinting_groups_repeats(db,super_admin_api):
 ...
 errors=super_admin_api.get(f'{BASE}/api/system-health',timeout=10).json()['recent_errors'];row=next(x for x in errors if x['message']==message);assert row['occurrences']==3
def test_recovery_transition_is_recorded(db,super_admin_api,monkeypatch):
 super_admin_api.get(f'{BASE}/api/system-health',timeout=10)
