...
const tours = {
  ...
  dashboard: async page=>{
    ...
    await note(page,'#opTimeProgress .op-card .op-dual-progress','Tiến độ theo công đoạn','Mỗi công đoạn có hai thước đo: thời gian thực tế so với định mức và sản phẩm đạt so với số lượng kế hoạch. So sánh hai thanh để phát hiện nơi dùng nhiều thời gian nhưng đầu ra thấp.',{action:'So sánh phần trăm thời gian với phần trăm sản phẩm của một công đoạn.',expected:'Xác định được công đoạn đúng tiến độ hoặc có nguy cơ chậm và cần quản lý kiểm tra.'});
