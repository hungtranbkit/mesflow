...
def test_dashboard_mobile_tabs_share_typography_tokens_and_disable_only_autosizing():
    css = UI_CSS.read_text(encoding='utf-8')
    pane = css.split('body[data-page="dashboard"] .dashboard-tab-pane{', 1)[1].split('}', 1)[0]
    assert '--dashboard-card-title:16px' in pane
    assert '--dashboard-body:14px' in pane
    assert '--dashboard-meta:12px' in pane
    ...
    hotfix = css.split('/* DASHBOARD-MOBILE-TAB-FONT-20260918', 1)[1]
    mobile = hotfix.split('@media(max-width:820px){', 1)[1]
    assert ':is(.op-card-identity,.emp-op-item)' in mobile
    assert 'font-size:var(--dashboard-card-title)' in mobile
    assert 'font-size:var(--dashboard-body)' in mobile
    assert 'font-size:var(--dashboard-meta)' in mobile
