...
def test_segment_tone_comes_from_the_operation_not_the_loop_index():
    ...
    # Vòng lặp vẽ thanh không còn nhận chỉ số -- không còn gì để lỡ tay dùng lại.
    render = js[js.index('${ordered.map('):]
    render = render[:render.index('employee-session-segment')]
    assert '(x,i)=>' not in render, 'vòng lặp vẽ thanh vẫn mang chỉ số'
