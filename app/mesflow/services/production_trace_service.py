...
class ProductionTraceService:
    ...
    def reconcile(self,kind,id):
        if kind=='session':
            current=fetch_one('SELECT good_qty,defect_qty,rework_qty,scrap_qty FROM work_sessions WHERE id=%s',(id,));where='session_id=%s';params=(id,)
        elif kind=='operation':
            ...
            current=fetch_one('SELECT COALESCE(SUM(done_qty),0) good_qty,COALESCE(SUM(defect_qty),0) defect_qty,COALESCE(SUM(rework_qty),0) rework_qty,COALESCE(SUM(scrap_qty),0) scrap_qty FROM operations WHERE production_order_id=%s AND COALESCE(is_rework_op,FALSE)=FALSE',(id,));where='production_order_id=%s';params=(id,)
