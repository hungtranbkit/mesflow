...
function poPartCard(p,index){const ops=window.poDetail.ops.filter(x=>Number(x.part_id)===Number(p.id)).filter(x=>window.poShowSupportOps||OpPolicy.isProduction(x)).sort((a,b)=>(a.sort_order||0)-(b.sort_order||0));return `<article class="part-block po-part-card"><div class="part-block-head po-part-head"><div><span class="part-block-badge po-part-index">Part ${index+1}</span><h2>${esc(p.code)} · ${esc(p.name)}</h2><small>${ops.length} Operation</small>${p.drawing_path?`<a class="template-drawing-link" href="/uploads/${esc(p.drawing_path)}" target="_blank">📎 Xem bản vẽ</a>`:''}</div><div class="panel-actions"><button class="btn" onclick="poPartModal(${p.id})">Sửa Part</button><button class="btn part-block-destructive" onclick="removePoPart(${p.id})">Xóa Part</button></div></div><div class="op-list po-op-list">${ops.length?`<div class="op-list-head po-op-header"><span>STT</span><span>Công đoạn</span><span>Kế hoạch</span><span>Đạt</span><span>NG / sửa</span><span>Trạng thái</span><span>QR</span><span>Thao tác</span></div>${ops.map((o,i)=>poOperationRow(o,i)).join('')}`:'<div class="empty-hint">Part này chưa có Operation.</div>'}</div><footer class="part-block-foot"><button class="btn part-block-add" onclick="poOperationModal(null,${p.id})">+ Thêm Operation</button></footer></article>`}
function poOperationRow(o,index){const src=window.poDetail?.ops?.find(x=>Number(x.id)===Number(o.input_source_operation_id)),kind=String(o.input_source_kind||'GOOD'),sourcePool=src?(kind==='REWORK'?Number(src.rework_qty||0):Number(src.done_qty||0)):0,allocated=src?(kind==='REWORK'?Number(src.rework_allocated_qty||0):Number(src.good_allocated_qty||0)):0,available=src?Math.max(sourcePool-allocated,0):Number(o.wip_qty||0),flow=(o.input_flow_enabled&&src)||Number(o.wip_qty||0)>0?`<small class="po-flow-note">WIP ${format(available)} · ${src?`từ ${esc(src.code)}`:esc(o.wip_source==='PLANNED_QUANTITY_FALLBACK'?'nguồn kế hoạch':'nguồn sản xuất')}</small>`:'',warning=['CRITICAL','WARNING','WAITING'].includes(String(o.control_state||''))?`<small class="po-op-warning">${esc(o.recommended_action||o.readiness_reason||'Cần kiểm tra')}</small>`:'';// SETUP là dòng con của OP chính: đánh dấu rõ, không lẫn vào routing sản xuất.
  ...
  const setupBadge=isSetup?'<span class="op-setup-badge">OP phụ · Setup máy</span>'
    // Nhãn trung tính: chỉ cho biết OP này CÓ việc setup kèm theo. Nhãn cũ ở
    // đây là một cảnh báo nền hổ phách -- ngôn ngữ của luật chặn đã bỏ.
    :(o.requires_setup?'<span class="op-setup-badge">Có OP setup</span>':'');
