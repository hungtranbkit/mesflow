...
const menu=[
  ...
  {label:'Hệ thống',items:[
    ...
    {page:'system-errors',label:'Lỗi hệ thống',hint:'HTTP 500, DB, service unhealthy — khác NG và ngoại lệ Session'},
...
async function renderDashboard(){
  ...
  const timeline=(sessions,date,view=dayView)=>{
    ...
    return `<div class="employee-day-legend"><span><i class="legend-session"></i>Session</span><span><i class="legend-gap"></i>Khoảng hở ≥15 phút</span><span><i class="legend-lunch"></i>${breaks.map(x=>x[2]).join(' · ')||'Không có giờ nghỉ'}</span><span><i class="legend-off"></i>Ngoài ca</span></div><div class="employee-day-head"><span>Nhân viên / ngày công</span><div class="employee-day-scale shift-scale">${scaleHtml}</div><span>Session / sản lượng</span></div><div class="employee-day-list">${workers.map(g=>{
      ...
      return `<article class="employee-day-row ${g.open?'open':''} ${g.seconds>=target?'complete':''} ${staleOpen?'stale-open':''}"><div class="employee-day-person"><b>${esc(g.employee_name||'—')}</b><small>${esc(g.employee_code||'')} · ${g.sessions.length} session${g.open?` · ${g.open} chưa đóng`:''}</small><div class="employee-day-progress"><i style="width:${percent}%"></i></div><strong>${duration(g.seconds)} / ${duration(target)}</strong><em>${over?`Vượt ${duration(over)}`:`Còn ${duration(remaining)}`}</em>${staleOpen?'<mark>Session mở quá cuối ca</mark>':''}</div><div class="employee-day-track-scroll" tabindex="0" role="region" aria-label="Timeline session trong ngày"><div class="employee-day-track shift-track"><div class="session-grid-lines">${gridHtml}</div><i class="shift-off before" style="left:${pct(viewStart)}%;width:${pct(workWindows[0][0])-pct(viewStart)}%"></i>${breaks.map(([a,b,label])=>`<i class="shift-lunch" style="left:${pct(a)}%;width:${pct(b)-pct(a)}%"><span>${esc(label)}</span></i>`).join('')}<i class="shift-off after" style="left:${pct(shiftEnd)}%;width:${pct(viewEnd)-pct(shiftEnd)}%"><span>Hết ca</span></i>${gapBlocks(ordered).map(([a,b,long])=>`<i class="employee-gap ${long?'long':''}" style="left:${pct(a)}%;width:${Math.max(.5,pct(b)-pct(a))}%" title="Khoảng hở ${duration((b-a)/1000)}"></i>`).join('')}${(()=>{const lanes=assignLanes(ordered),laneCount=Math.max(1,...lanes.map(n=>n+1));
...
function drawProductionOrderDetail(){
  ...
  // COMPLETED (a SETUP session closes at good_qty=0), so counting one leaves
...
function userModal(user=null){
  // Task spec section 4: only a Super Admin session may grant/keep the
...
async function renderSessionManagement(){
  ...
  const loadSessions=async(preserveScroll=true,requestedState=null)=>{const sequence=++loadSequence,scrollX=window.scrollX,scrollY=window.scrollY;el('smSessionList').innerHTML=MFUI.loadingState('Đang tải Session…');try{const requested=requestedState||filterState(),normalized=await loadFilterOptions(requested,sequence);if(!normalized||sequence!==loadSequence)return;syncSessionUrl();const filterParams={po_id:normalized.po,part_id:normalized.part,operation_id:normalized.operation,employee_id:normalized.employee},day=el('smDate').value,sessionQs=new URLSearchParams({...filterParams,status:el('smStatus').value,from:day,to:day,limit:'500'});sessionReport=await api('/api/session-management?'+sessionQs);if(sequence!==loadSequence)return;drawSessions();if(returnContext){requestAnimationFrame(()=>document.querySelector(`[data-session-id="${Number(returnContext.sessionId)}"]`)?.scrollIntoView({behavior:'smooth',block:'center'}))}else if(preserveScroll)requestAnimationFrame(()=>window.scrollTo(scrollX,scrollY))}catch(e){if(sequence!==loadSequence)return;el('smSessionList').innerHTML=MFUI.errorState(e.message||'Không tải được Session.','smRetry');el('smRetry').onclick=()=>loadSessions(false)}};
  ...
  const openExcludeRestore=()=>{
    ...
    el('exHelp').textContent=excluded?'Session sẽ được tính lại vào thời gian, sản lượng, KPI và tiến độ Operation.':'Session vẫn còn trong lịch sử/audit; chỉ không tính thời gian, sản lượng, KPI và tiến độ Operation nữa.';
  ...
  el('exSave').onclick=async()=>{
    ...
    try{
      ...
      toast(excluded?'Đã khôi phục Session vào báo cáo':'Đã loại Session khỏi báo cáo');closeExcludeRestore();await loadSessions(true)
