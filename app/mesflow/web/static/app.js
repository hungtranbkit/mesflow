  ...
  const timeline=(sessions,date,view=dayView)=>{
    ...
    const sessionChip=x=>{
      ...
      return MFUI.opIdentity({
        name:x.operation_name,code:x.operation_code,compact:true,showCode:false,
        tooltip:full,metaHtml:`${esc(when)} · ${qty}`,className:open?'open':'',
      });
    };
...
function poPartCard(p,index){const ops=window.poDetail.ops.filter(x=>Number(x.part_id)===Number(p.id)).filter(x=>window.poShowSupportOps||OpPolicy.isProduction(x)).sort((a,b)=>(a.sort_order||0)-(b.sort_order||0));return `<article class="part-block po-part-card"><div class="part-block-head po-part-head"><div class="part-identity"><h2>${esc(p.name||p.code)}</h2><small class="part-meta">${[
    p.code?`<span class="part-meta-code">${esc(p.code)}</span>`:'',
    `<span class="part-meta-ops">${ops.length} Operation</span>`,
  ].filter(Boolean).join('<span class="part-meta-sep" aria-hidden="true">·</span>')}</small>${p.drawing_path?`<a class="template-drawing-link" href="/uploads/${esc(p.drawing_path)}" target="_blank">📎 Xem bản vẽ</a>`:''}</div><div class="panel-actions"><button class="btn" onclick="poPartModal(${p.id})">Sửa Part</button><button class="btn part-block-destructive" onclick="removePoPart(${p.id})">Xóa Part</button></div></div><div class="op-list po-op-list">${ops.length?`<div class="op-list-head po-op-header"><span>STT</span><span>Công đoạn</span><span>Kế hoạch</span><span>Đạt</span><span>NG / sửa</span><span>Trạng thái</span><span>QR</span><span>Thao tác</span></div>${ops.map((o,i)=>poOperationRow(o,i)).join('')}`:'<div class="empty-hint">Part này chưa có Operation.</div>'}</div><footer class="part-block-foot"><button class="btn part-block-add" onclick="poOperationModal(null,${p.id})">+ Thêm Operation</button></footer></article>`}
