async function renderOverview(){
  ...
  // it; only otherwise does the neutral "Đã dừng HH:mm" line name who worked
  // it earlier today. Both reset by themselves at local midnight.
  const workerChips=(list,tip,cls='')=>{const shown=list.slice(0,3),rest=list.slice(3);return `${shown.map(w=>`<span class="ov-worker${cls}" title="${E(tip(w))}"><i></i><b>${E(w.name||w.employee_no||'—')}</b>${w.employee_no?`<small>${E(w.employee_no)}</small>`:''}${Number(w.session_count)>1?`<small class="ov-worker-tag">×${Number(w.session_count)}</small>`:''}</span>`).join('')}${rest.length?`<span class="ov-worker more" title="${E(rest.map(tip).join('\n'))}"><b>+${rest.length}</b></span>`:''}`};
  const todayTip=w=>[w.name,w.employee_no,w.last_ended_at?`dừng ${T(w.last_ended_at)}`:'',w.auto_closed?'tự đóng khi hết ca':'',Number(w.session_count)>1?`${w.session_count} phiên hôm nay`:''].filter(Boolean).join(' · ');
  const todayWorkers=x=>{const list=Array.isArray(x.today_worker_list)?x.today_worker_list:[];if(x.today_state!=='STOPPED'||!list.length)return '';const at=x.today_last_ended_at?T(x.today_last_ended_at):'';return `<div class="overview-op-workers is-stopped" aria-label="${list.length} người đã làm hôm nay"><em>${at?`Đã dừng ${at}`:'Đã làm hôm nay'}</em>${workerChips(list,todayTip,' stopped')}</div>`};
  const dur=sec=>{sec=Math.max(0,Math.round(Number(sec)||0));const h=Math.floor(sec/3600),m=Math.floor(sec%3600/60);return h?`${h} giờ ${String(m).padStart(2,'0')} phút`:`${m} phút`};
  // So định mức: same formula, rounding and wording as Operation Detail /
  // Báo cáo năng suất -- AVG of định mức × (Đạt + Lỗi) ÷ thời gian thực tế × 100
  // over today's CLOSED sessions, computed server-side.
  const todaySpeed=pct=>pct==null?{text:'Chưa đủ dữ liệu',cls:'na'}:pct>100?{text:'Nhanh hơn định mức',cls:'fast'}:pct<100?{text:'Chậm hơn định mức',cls:'slow'}:{text:'Đúng định mức',cls:'target'};
  const todayMetrics=x=>{const t=x.today;if(!t||!Number(t.session_count))return '<div class="overview-op-today is-empty" data-today-metrics><small>Hôm nay</small><span>Chưa có phiên làm việc</span></div>';
    const pct=t.productivity_percent==null?null:Number(t.productivity_percent),sp=todaySpeed(pct),standard=Number(t.standard_seconds_per_unit||0);
    const pctText=pct==null?(standard>0?'—':'Chưa có định mức'):`${pct.toLocaleString('vi-VN',{minimumFractionDigits:1,maximumFractionDigits:1})}%`;
    const pctTip=`So định mức: ${pct==null?(standard>0?'Chưa đủ dữ liệu (chưa có phiên đã kết thúc có sản lượng)':'Chưa cấu hình định mức'):sp.text} · định mức × (Đạt + Lỗi) ÷ thời gian thực tế × 100%, trung bình ${N(t.scored_session_count)} phiên đã kết thúc hôm nay`;
    const m=(k,label,value,tip='',cls='')=>`<span class="ov-today-m${cls}" data-today="${k}"${tip?` title="${E(tip)}"`:''}><small>${label}</small><b>${value}</b></span>`;
    return `<div class="overview-op-today" data-today-metrics aria-label="Hôm nay"><small class="ov-today-label">Hôm nay</small>${m('employees','NV',N(t.employee_count),'Nhân viên có phiên làm việc hôm nay')}${m('sessions','Phiên',N(t.session_count),`${N(t.open_session_count)} đang mở`)}${m('good','Đạt',N(t.good_qty))}${m('defect','Lỗi',N(t.defect_qty))}${m('rework','Sửa',N(t.rework_qty),'Lỗi sửa được')}${m('time','Thời gian',dur(t.work_seconds),'Thời gian làm thực tế trong giờ làm việc (lịch làm việc), như Dashboard theo ngày')}${m('productivity','Năng suất',pctText,pctTip,` op-speed ${pct==null?'na':sp.cls}`)}</div>`};
  const operationRows=poId=>mergedOps().filter(x=>Number(x.po_id)===Number(poId)).sort((a,b)=>Number(a.part_id)-Number(b.part_id)||Number(a.operation_sort)-Number(b.operation_sort)).map(x=>`<div class="overview-op-row ${Number(x.repair_pending_quantity)>0?'has-repair':''}" data-repair-op="${x.operation_id}" data-op-detail="${x.operation_id}" tabindex="0" role="button" aria-label="Mở chi tiết Operation ${E(x.operation_code)}" title="Nhấp đúp để xem người đã làm và năng suất"><span><b class="row-title">${E(x.operation_name)}</b><small class="row-code">${E(x.operation_code)}</small><small>${E(x.part_code)} ${E(x.part_name||'')}</small></span><span><b>${Number(x.progress_percent||0).toFixed(1)}%</b>${progress(x.progress_percent,x.control_state)}</span><span><small>Đạt</small><b>${N(x.done_qty)}</b></span><span><small>Lỗi</small><b>${N(x.defect_qty)}</b></span><span><small>Chờ sửa</small><b>${N(x.repair_pending_quantity)}</b><small>${Number(x.repair_pending_quantity)>0?work(x.estimated_repair_work_seconds):'—'}</small></span><span><button class="btn mini" data-open-op="${x.operation_id}" type="button">Mở OP</button></span>${todayMetrics(x)}${activeWorkers(x)||todayWorkers(x)}</div>`).join('');
  const showOperationDetail=async(operationId,opts={})=>{
    ...
    const bindManualForm=()=>{
      ...
      // Lỗi sửa được is a subset of Lỗi (work_sessions CHECK rework_qty<=defect_qty):
      // say so as soon as it is typed, not only after a round trip.
      const reworkError='Lỗi sửa được không được lớn hơn số Lỗi.';
      const reworkTooBig=()=>form.rework_qty.value!==''&&form.defect_qty.value!==''&&Number(form.rework_qty.value)>Number(form.defect_qty.value);
      form.oninput=()=>{preview();showError(reworkTooBig()?reworkError:'')};
      ...
      form.onsubmit=async e=>{
        ...
        const employeeId=Number(form.employee_id.value),st=form.started_at.value,en=form.ended_at.value,good=Number(form.good_qty.value),defect=Number(form.defect_qty.value),rework=Number(form.rework_qty.value),reason=String(form.reason.value||'').trim(),note=String(form.note.value||'').trim();
        ...
        if(form.good_qty.value===''||form.defect_qty.value===''||form.rework_qty.value===''||![good,defect,rework].every(v=>Number.isInteger(v)&&v>=0))return showError('Sản lượng phải là số nguyên không âm.');
        if(rework>defect)return showError(reworkError);
        ...
        try{
          ...
          const res=await api('/api/supervisor/sessions/manual',{method:'POST',body:JSON.stringify({operation_id:Number(operationId),employee_id:employeeId,started_at:st,ended_at:en,good_qty:good,defect_qty:defect,rework_qty:rework,reason,note,request_id:form.dataset.requestId})});
          const created=res.session||{};
          toast(`Đã bổ sung phiên #${created.id||''} · ${N(good)} đạt · ${N(defect)} lỗi${rework>0?` (sửa được ${N(rework)})`:''}`);
          await reloadDetail(created.id);
          if(typeof load==='function')load();
        }catch(err){const msg=err.message||String(err);showError(/rework_qty cannot exceed defect_qty/.test(msg)?reworkError:msg);form.dataset.saving='';submit.disabled=false;submit.textContent='Lưu phiên bổ sung'}
      };
    };
    ...
    try{
      ...
      sessions.forEach(s=>{
        ...
        if(!people.has(key))people.set(key,{employee_id:s.employee_id,code:s.employee_code||'',name:s.employee_name||'Không rõ',sessions:0,open:0,good:0,defect:0,rework:0,worked:0,scores:[]});
        const p=people.get(key);p.sessions+=1;p.open+=String(s.status||'').toUpperCase()==='OPEN'?1:0;p.good+=Number(s.good_qty||0);p.defect+=Number(s.defect_qty||0);p.rework+=Number(s.rework_qty||0);p.worked+=Number(s.duration_seconds||0);
      ...
      const employeeRows=employees.length?employees.map(p=>{const st=speed(p.productivity);return `<div class="op-detail-user-row"><span><b>${E(p.name)}</b><small>${E(p.code)}${p.open?` · ${p.open} đang làm`:''}</small></span><span><b>${N(p.sessions)}</b><small>phiên làm việc</small></span><span><b>${N(p.good)}</b><small>Đạt · ${N(p.defect)} lỗi${p.rework>0?` · sửa được ${N(p.rework)}`:''}</small></span><span><b>${work(p.worked)}</b><small>thời gian thực tế</small></span><span><b>${p.productivity==null?'—':`${p.productivity.toFixed(1)}%`}</b><small class="op-speed ${st.cls}">${st.text}</small></span>${canManualSession?`<span class="op-manual-cell">${p.employee_id?`<button class="btn mini op-manual-add" type="button" data-manual-session-employee="${Number(p.employee_id)}" title="Bổ sung phiên cho ${E(p.name)}" aria-label="Bổ sung phiên cho ${E(p.name)}">+ Phiên</button>`:''}</span>`:''}</div>`}).join(''):'<div class="op-detail-empty">Chưa có nhân viên nào làm Operation này.</div>';
      ...
      const manualForm=`<form class="op-manual-form" data-manual-session-form hidden novalidate><div class="op-manual-title"><b>Bổ sung phiên làm việc</b><span>Cho trường hợp quên quét QR bắt đầu/kết thúc · tạo phiên CLOSED thật · có audit</span></div><div class="op-manual-fields"><label class="wide"><span>Nhân viên <b>*</b></span><select name="employee_id" required><option value="">Đang tải danh sách…</option></select></label><label><span>Bắt đầu <b>*</b></span><input name="started_at" type="datetime-local" step="60" required></label><label><span>Kết thúc <b>*</b></span><input name="ended_at" type="datetime-local" step="60" required></label><label><span>Sản lượng Đạt</span><input name="good_qty" type="number" min="0" step="1" inputmode="numeric" value="0" required></label><label><span>Lỗi</span><input name="defect_qty" type="number" min="0" step="1" inputmode="numeric" value="0" required></label><label><span>Lỗi sửa được</span><input name="rework_qty" type="number" min="0" step="1" inputmode="numeric" value="0" required title="Trong số Lỗi, bao nhiêu sản phẩm sửa được (không lớn hơn Lỗi)"></label><div class="op-manual-preview"><small>So định mức</small><output name="score_preview" data-manual-score>—</output></div><div class="op-manual-preview wide"><small>Thời gian</small><output name="duration_preview" data-manual-duration>—</output></div><label class="wide"><span>Lý do bổ sung <b>*</b></span><input name="reason" type="text" maxlength="500" required placeholder="Ví dụ: công nhân quên quét QR kết thúc"></label><label class="full"><span>Ghi chú <small>(không bắt buộc)</small></span><input name="note" type="text" maxlength="500"></label></div><div class="op-manual-error" data-manual-error role="alert" hidden></div><div class="op-manual-actions"><button class="btn" type="button" data-manual-session-cancel>Hủy</button><button class="btn primary" type="submit" data-manual-session-submit>Lưu phiên bổ sung</button></div></form>`;
