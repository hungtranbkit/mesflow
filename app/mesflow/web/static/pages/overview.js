  ...
  const showOperationDetail=async operationId=>{
    ...
    try{
      ...
      const sessionRows=visibleSessions.map(s=>{
        ...
        const action=canAdjustSessionQty&&closed?`<button class="btn mini ${needsQty?'primary':''}" type="button" data-edit-session-qty="${s.session_id}">${needsQty?'Bổ sung SL':'Sửa SL'}</button>`:'';
        const editor=canAdjustSessionQty&&closed?`<form class="op-session-qty-editor" data-session-qty-form="${s.session_id}" hidden><div class="op-session-edit-title"><b>Chỉnh sản lượng phiên #${s.session_id}</b><span>Lưu qua cơ chế SESSION_ADJUST · có audit</span></div><div class="op-session-edit-fields"><label>Đạt<input name="good_qty" type="number" min="0" step="1" value="${Number(s.good_qty||0)}" required></label><label>Lỗi<input name="defect_qty" type="number" min="0" step="1" value="${Number(s.defect_qty||0)}" required></label><label>Trong đó sửa lại<input name="rework_qty" type="number" min="0" step="1" value="${Number(s.rework_qty||0)}" required></label><label class="reason">Lý do chỉnh sửa<input name="reason" type="text" maxlength="500" placeholder="${needsQty?'Bổ sung sản lượng phiên 0/0':'Nhập lý do thay đổi sản lượng'}" required></label></div><div class="op-session-edit-actions"><button class="btn" type="button" data-session-edit-cancel>Hủy</button><button class="btn primary" type="submit">Lưu sản lượng</button></div></form>`:'';
      ...
      modal.querySelectorAll('[data-session-qty-form]').forEach(form=>form.onsubmit=async e=>{
        ...
        if(!reason)return alert('Phải nhập lý do chỉnh sửa để lưu audit.');
