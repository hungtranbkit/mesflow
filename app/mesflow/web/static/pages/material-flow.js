(function(){
  ...
  function historyLabel(h){
    ...
    if(h.action==='DELETE')return `Đã xóa lần cấp đầu vào #${h.ledger_id}`;
    ...
    return changes.join(' · ')||'Cập nhật thông tin cấp đầu vào';
  }
  ...
  window.openMaterialFlowTrace=async function(operationId){
    ...
    title.textContent='Đang tải Dòng vật tư…';sub.textContent='';body.innerHTML='<div class="empty">Đang đọc cấu hình và lịch sử cấp đầu vào…</div>';
    try{
      ...
      body.innerHTML=`
        ...
        <section class="mf-section"><div class="mf-summary">
          ...
          <div><small>GOOD còn khả dụng</small><b>${F(s.good_available_qty)}</b></div><div><small>REWORK còn khả dụng</small><b>${F(s.rework_available_qty)}</b></div><div><small>Phế</small><b>${F(s.scrap_qty)}</b></div><div><small>Số lần cấp đầu vào</small><b>${F(s.ledger_count)}</b></div>
        </div></section>
        <section class="mf-section"><h3>Operation nhận đầu ra từ OP này</h3>${downstreamHtml(downstream)}</section>
        <section class="mf-section"><div class="mf-section-head"><div><h3>Lịch sử cấp đầu vào</h3><p>Mỗi dòng ghi nhận một lần phiên làm việc của OP đích nhận đầu vào.</p></div><span>${items.length} dòng</span></div>
          <div class="table-wrap"><table class="data-table"><thead><tr><th>Hướng</th><th>Nguồn</th><th>Đích</th><th>Pool</th><th>Phiên làm việc</th><th>Nhân viên</th><th>SL tốt</th><th>SL lỗi</th><th>Nguồn gốc</th><th>Cập nhật</th></tr></thead><tbody>${items.length?items.map(x=>`<tr><td><span class="badge ${Number(x.source_operation_id)===Number(operationId)?'ok':'muted'}">${Number(x.source_operation_id)===Number(operationId)?'CẤP RA':'NHẬN VÀO'}</span></td><td><b>${E(x.source_code)}</b><br><small>${E(x.source_name)}</small></td><td><b>${E(x.target_code)}</b><br><small>${E(x.target_name)}</small></td><td>${E(kindLabel(x.source_qty_kind))}</td><td>#${x.session_id}<br><small>${E(x.session_status)}</small></td><td>${E(x.employee_no||'')}<br><small>${E(x.employee_name||'')}</small></td><td>${F(x.good_qty_consumed)}</td><td>${F(x.defect_qty_consumed)}</td><td><span class="badge">${E(x.origin||'RUNTIME')}</span></td><td>${DT(x.updated_at)}</td></tr>`).join(''):'<tr><td colspan="10"><div class="empty compact">Chưa phát sinh lần cấp đầu vào cho Operation này.</div></td></tr>'}</tbody></table></div></section>
        <section class="mf-section"><div class="mf-section-head"><div><h3>Lịch sử thay đổi</h3><p>RUNTIME = kiosk/phiên làm việc · BACKFILL = dữ liệu nâng cấp · ADMIN_EDIT = chỉnh phiên làm việc.</p></div><span>${history.length} thay đổi</span></div><div class="mf-history">${history.length?history.map(h=>`<article><b>${E(h.action)}</b><span>${E(historyLabel(h))}</span><small>${DT(h.changed_at)}</small></article>`).join(''):'<div class="empty compact">Chưa có thay đổi nào trong lịch sử cấp đầu vào.</div>'}</div></section>`;
    }catch(e){
