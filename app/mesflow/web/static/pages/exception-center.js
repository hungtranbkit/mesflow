...
const ExceptionCenter=(()=>{
  ...
  const labels={LONG_OPEN_SESSION:'Phiên làm việc mở quá lâu',ZERO_QUANTITY_LONG:'Sản lượng bất thường',MISSING_STATION:'Thiếu thông tin trạm',INVALID_DURATION:'Thời gian không hợp lệ',OPERATION_COMPLETED_SESSION_OPEN:'Operation hoàn tất nhưng phiên làm việc còn mở',EMPLOYEE_SESSION_CONFLICT:'Phiên làm việc xung đột',INTERRUPTED_QUANTITY_ENTRY:'Quét thẻ mới trong khi chưa nhập số liệu'};
  ...
  let ecLoaded=false;
  async function load(quiet=false){try{const data=await api('/api/exceptions?'+query());state.items=data.items||[];const high=state.items.filter(x=>['HIGH','CRITICAL'].includes(x.severity)).length;document.getElementById('ecSummary').innerHTML=`<b>${data.total} trường hợp bất thường</b>${high?`<span class="ec-severity-badge">${high} mức cao/nghiêm trọng</span>`:''}<small>Cập nhật ${new Intl.DateTimeFormat('vi-VN',{timeStyle:'medium',hour12:false}).format(new Date())}</small>`;document.getElementById('ecList').innerHTML=state.items.length?state.items.map(card).join(''):`<div class="empty"><b>Không có phiên làm việc bất thường trong nhóm này</b><span>Hệ thống vẫn tiếp tục đối soát định kỳ.</span></div>`;bindCards();ecLoaded=true}catch(e){MFUI.refreshError({loaded:quiet||ecLoaded,host:document.getElementById('ecList'),error:e,retry:()=>load(false),screen:'Các phiên làm việc bất thường'})}}
  ...
  function tabOverview(r){
    ...
    return `<section><h3>Thông tin bất thường</h3><dl><dt>Loại</dt><dd>${esc(x.title||labels[x.exception_type]||x.exception_type)}</dd><dt>Mức độ</dt><dd>${esc(x.severity)}</dd><dt>Trạng thái</dt><dd>${esc(statusLabel[x.status]||x.status)}</dd><dt>Mô tả</dt><dd>${esc(x.message)}</dd><dt>Cần làm</dt><dd>${esc(x.recommended_action||'—')}</dd></dl></section>
      ...
      <section><h3>Thời gian và sản lượng</h3><div class="ec-metrics"><span><small>Bắt đầu</small><b>${fmt(s.started_at)}</b></span><span><small>Kết thúc</small><b>${fmt(s.ended_at)}</b></span><span><small>Thời lượng</small><b>${fmtDuration(s.duration_seconds)}</b></span><span><small>Đạt</small><b>${MFUI.qtyValue(s.good_qty,mfOutputRecorded(s))}</b></span><span><small>Lỗi</small><b>${MFUI.qtyValue(s.defect_qty,mfOutputRecorded(s))}</b></span><span><small>Sửa được</small><b>${MFUI.qtyValue(s.rework_qty,mfOutputRecorded(s))}</b></span></div></section>`;
  }
  function tabAdjust(r){
    if(!r.editableFields.length)return `<section><p class="ec-empty-note">Trường hợp bất thường này không cho phép điều chỉnh trực tiếp dữ liệu phiên làm việc theo quy tắc nghiệp vụ hiện tại. Chọn "Bỏ qua" nếu đã xác nhận dữ liệu hợp lệ, hoặc "Mở phiên làm việc đầy đủ" để thực hiện thay đổi khác.</p></section>`;
    const rows=diffRows(r);
    return `<section><h3>Sửa phiên làm việc #${r.session.session_id}</h3><div class="ec-field-grid">${r.editableFields.map(f=>fieldInput(r,f)).join('')}</div></section>
  ...
  function tabVerify(r){
    return `<section><h3>Dòng thời gian kiosk</h3><ol class="ec-timeline"><li><time>${fmt(r.session.started_at)}</time><span>Bắt đầu phiên làm việc</span></li>${(r.activity||[]).map(x=>`<li><time>${fmt(x.occurred_at)}</time><span>${esc(x.message||x.event_type)}</span></li>`).join('')}</ol></section>`;
  }
  ...
  function renderResolution(){
    ...
    node.innerHTML=`<button class="ec-drawer-backdrop" aria-label="Đóng"></button><aside class="ec-drawer ec-resolution" role="dialog" aria-modal="true" aria-label="Xử lý phiên làm việc bất thường"><header><div><small>Phiên làm việc #${x.session_id||'—'}</small><h2>${esc(s.employee_name||x.employee_name||'Trường hợp bất thường')}</h2><span class="ec-status">${esc(statusLabel[x.status]||x.status)}</span></div><button id="ecClose" aria-label="Đóng">×</button></header>
      ...
      <footer><textarea id="ecReason" class="ec-reason" placeholder="Ghi lý do xử lý / điều chỉnh (bắt buộc để lưu điều chỉnh, hoàn tất hoặc bỏ qua)">${esc(r.reasonDraft||'')}</textarea><div class="ec-actions">${x.session_id?'<button class="btn" data-action="open-full">Mở phiên làm việc đầy đủ</button>':''}${canAck?'<button class="btn" data-action="take">Nhận xử lý</button>':''}${canAdjust?'<button class="btn" data-action="save">Lưu điều chỉnh</button>':''}<button class="btn primary" data-action="resolve" ${canResolve?'':'disabled'} ${!canResolve&&stillOpen?'title="Ngoại lệ vẫn còn hiệu lực -- sửa phiên làm việc hoặc kiểm tra lại trước khi hoàn tất"':''}>Hoàn tất xử lý</button><button class="btn danger" data-action="ignore" ${stillOpen?'':'disabled'}>Bỏ qua</button><button class="btn" data-action="close">Đóng</button></div></footer></aside>`;
  ...
  async function saveCorrection(){
    ...
    try{
      ...
      if(err.status===409&&payload.error==='SESSION_CHANGED'){
        ...
        r.banner={type:'danger',text:'Phiên làm việc đã được người khác thay đổi trong lúc bạn đang xử lý. Dữ liệu đã được làm mới -- vui lòng kiểm tra lại trước khi lưu.'};
  ...
  function openSessionInManagement(item){
    if(!item?.session_id){toast('Trường hợp bất thường này không gắn với một phiên làm việc cụ thể.');return}
