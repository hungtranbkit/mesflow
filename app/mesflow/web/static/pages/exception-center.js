...
const ExceptionCenter=(()=>{
  ...
  let ecLoaded=false;
  async function load(quiet=false){try{const data=await api('/api/exceptions?'+query());state.items=data.items||[];const high=state.items.filter(x=>['HIGH','CRITICAL'].includes(x.severity)).length;document.getElementById('ecSummary').innerHTML=`<b>${data.total} trường hợp bất thường</b>${high?`<span class="ec-severity-badge">${high} mức cao/nghiêm trọng</span>`:''}<small>Cập nhật ${new Intl.DateTimeFormat('vi-VN',{timeStyle:'medium'}).format(new Date())}</small>`;document.getElementById('ecList').innerHTML=state.items.length?state.items.map(card).join(''):`<div class="empty"><b>Không có phiên làm việc bất thường trong nhóm này</b><span>Hệ thống vẫn tiếp tục đối soát định kỳ.</span></div>`;bindCards();ecLoaded=true}catch(e){MFUI.refreshError({loaded:quiet||ecLoaded,host:document.getElementById('ecList'),error:e,retry:()=>load(false),screen:'Các phiên làm việc bất thường'})}}
