...
const ExceptionCenter=(()=>{
  ...
  function tabOverview(r){
    ...
    return `<section><h3>Thông tin bất thường</h3><dl><dt>Loại</dt><dd>${esc(x.title||labels[x.exception_type]||x.exception_type)}</dd><dt>Mức độ</dt><dd>${esc(x.severity)}</dd><dt>Trạng thái</dt><dd>${esc(statusLabel[x.status]||x.status)}</dd><dt>Mô tả</dt><dd>${esc(x.message)}</dd><dt>Cần làm</dt><dd>${esc(x.recommended_action||'—')}</dd></dl></section>
      ...
      <section><h3>Thời gian và sản lượng</h3><div class="ec-metrics"><span><small>Bắt đầu</small><b>${fmt(s.started_at)}</b></span><span><small>Kết thúc</small><b>${fmt(s.ended_at)}</b></span><span><small>Thời lượng</small><b>${fmtDuration(s.duration_seconds)}</b></span><span><small>Đạt</small><b>${MFUI.qtyValue(s.good_qty,mfOutputRecorded(s))}</b></span><span><small>NG</small><b>${MFUI.qtyValue(s.defect_qty,mfOutputRecorded(s))}</b></span><span><small>Sửa được</small><b>${MFUI.qtyValue(s.rework_qty,mfOutputRecorded(s))}</b></span></div></section>`;
  }
