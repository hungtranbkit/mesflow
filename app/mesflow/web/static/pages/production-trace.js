const ProductionTrace=(()=>{
  ...
  const cats=[['','Tất cả'],['PO,OPERATION','Sản xuất'],['SESSION','Phiên làm việc'],['QUANTITY','Sản lượng'],['DEFECT','Lỗi'],['REWORK','Rework'],['EXCEPTION','Ngoại lệ'],['CHANGE','Thay đổi']];
  const qLabel={GOOD:'Đạt',DEFECT:'Lỗi',REPAIRABLE:'Sửa được',REWORK_RECOVERED:'Đã phục hồi',SCRAP:'Phế'};
  ...
  const item=e=>`<article class="trace-event cat-${e.category.toLowerCase()}"><time>${new Intl.DateTimeFormat('vi-VN',{timeZone:'Asia/Ho_Chi_Minh',timeStyle:'medium',hour12:false}).format(new Date(e.occurred_at))}</time><i></i><div><header><b>${esc(e.title)}</b><span>${esc(e.category)}</span>${e.source==='LEGACY_DERIVED'?'<em>Dữ liệu suy ra</em>':''}</header><p>${esc(e.description||'')}</p>${e.quantity_delta!=null?`<strong class="trace-qty ${e.quantity_delta<0?'negative':''}">${e.quantity_delta>0?'+':''}${e.quantity_delta}</strong>`:''}${e.category==='CHANGE'?auditDiff(e):''}<small>${esc(e.actor_name||'Hệ thống')}${e.operation_id?` · OP #${e.operation_id}`:''}${e.session_id?` · phiên làm việc #${e.session_id}`:''}</small>${technical(e.metadata)}</div></article>`;
