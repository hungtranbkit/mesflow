# Audit file Router thật — bản đồ field USED / UNUSED / DERIVED
...
⇒ Phần B/D/E cần **một migration** mới lưu được. Phần hiển thị ở màn xem trước
thì không cần migration và làm được ngay.

## PO creation on import

Hiện tại: import tạo **Template**; PO chỉ sinh khi bấm *Tạo Production Order*
(`/api/templates/<id>/instantiate`). File có `PO NUMBER = 6126` nên theo hợp đồng
mới, import phải tạo luôn PO. Đây là thay đổi luồng, không phải thêm field: cần
quyết định điểm idempotency (theo mã PO), hành vi khi PO đã tồn tại, và quan hệ
giữa `QTY 110` (PO) với `SỐ LƯỢNG` theo sheet (Part).
