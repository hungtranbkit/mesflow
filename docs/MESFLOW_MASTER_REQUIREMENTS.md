...
- **Operation identity**: `operation.id` is the canonical *technical* identity. **Business uniqueness is scoped to the Part**: `(part_id, op code / source_op_no)`.
  - The same OP number in **two different Parts** is **valid** — every drawing renumbers from OP01. No error, no warning.
  - The same OP number twice in **one Part** is a **BLOCKING ERROR**: a Part cannot have two operations numbered 02. The whole import is refused and **no Template, PO or Operation is created or updated**. Fixed message: `Lỗi: OP trùng mã trong cùng một Part. Vui lòng sửa lại file Excel.`, listing the sheet, the Part, the OP code and **every** conflicting row.
  - **No suffix is minted** (`-OP02-2`) to make it pass: that legalises a data error and puts an Operation into production under a code that appears nowhere in the customer's document. There is no override in the normal flow.
  - The OP number and title are still kept **verbatim as printed** (`source_op_no`, `source_title`) — they are business/display data, not identity.
- **Two kinds of sheet, both valid**: a sheet that declares `TÊN BẢN VẼ` / `MÃ BẢN VẼ` is a **DRAWING_PART_SHEET**; one that declares neither is a **PROCESS_LEVEL_SHEET** — a process step (welding, painting, assembly) that belongs to no single drawing. Drawing identity is **optional**: a process-level sheet is normal and **must never raise a warning** for lacking it, and no synthetic `PART-xx` code is ever shown to the user. Only a sheet that has a drawing **name** but no **code** is warned about (`DRAWING_SHEET_MISSING_CODE`) — there the identity is half-filled, which is a real mistake.
- **Process sheet with no OPERATION block**: never silently skipped. The import warns `Sheet '<name>' không có block OPERATION; MESFlow sẽ tạo 1 Operation quy trình từ tên sheet.` and creates **exactly one** process Operation named from the sheet, with no `source_op_no` (there is no number printed on the paper to keep). Skipping it made the sheet vanish from the PO, so the step could neither be scheduled nor scanned.
- **Drawing code is persisted, not just parsed**: `MÃ BẢN VẼ` is the technical identity the whole workshop calls a part by — the operator reads it off the paper drawing, the store sticks it on the crate, QC writes it on the slip. It is stored **verbatim** in `template_parts.drawing_code` (unlike `code`, which is character-filtered to be a safe internal key) and shown in Template detail (editable) and PO detail (read-only). `parts.drawing_code` is a **snapshot taken when the PO is instantiated**, not a read-through to the Template: a Template stays editable after its POs are running, and editing it must never silently rewrite the document of a PO already in production. The snapshot is **not retroactive** — Parts created before migration `0053` keep `NULL`, which states the truth that the system did not know the code then. A process-level sheet with no drawing keeps `NULL` too; nothing is invented.
...
| REQ-UI-024 | **A Part is named by its Vietnamese name, not by its position in a list.** The shared Part card (Template detail and PO detail) leads with the Part's Vietnamese NAME as the primary, largest text; the Part code, the drawing code and the operation count sit beneath it as one secondary, muted meta line whose items share a common vertical centre. The label `Part 1` / `Part 2` is never rendered — it is an index of the screen, not the name of anything on the shop floor, and it pushed the one string the user is actually scanning for (`Chân ghế A  - Trái`) behind a number and a machine code. Both screens use the same markup and the same CSS; they may not drift. |
| REQ-UI-025 | **Selecting something must be visible, focusable and announced.** The Template list is a list of SEPARATE cards — each with its own border and rounded corners, separated by a vertical `gap` from the spacing scale, never a single block divided by hairlines. The selected card carries `aria-current="true"` (the attribute the CSS selected state and the tests both key on; `aria-selected` is invalid on a `button` role and would be ignored by assistive technology), and reads as selected through more than one channel: a different surface, a stronger border and a colour stripe. Keyboard focus draws a visible `:focus-visible` outline. A list card is NOT a card surface for the purposes of the canonical `[class$="-card"]` sweep — that rule sets `background:...!important` and silently overrode every selected state, which is the defect this requirement closes. |
| REQ-UI-026 | **Exporting a file belongs to the record being exported.** The Production Order LIST has no export button: the one that used to be there downloaded `/api/operations/export.xlsx` — every Operation of every PO, which is nobody's document. Export starts from a single PO's detail screen (`Xuất Excel + QR`, REQ-TPL-006). |
...
| REQ-TPL-007 (Router import: full field semantics + PO creation) | `tests/integration/test_part_drawing_code.py` (MÃ BẢN VẼ persisted verbatim, snapshot at instantiate, not retroactive, survives a Template tree save), `tests/integration/test_migration_0053_part_drawing_code.py`, `tests/test_router_import_field_semantics.py` (36 cases: s/min/hour units and variants, the 60 min / 180 s / 6.5 h example, 0 vs `-` vs blank, total preserved when it disagrees, Part qty independent of PO QTY), `tests/test_router_po_import_flow.py`, `tests/integration/test_template_op_code_scoped_by_part.py` (same-Part duplicate -> blocking error naming sheet/Part/code/all rows; across Parts -> valid), `tests/integration/test_migration_0052_router_source_semantics.py`, `reports/ROUTER_IMPORT_FIELD_AUDIT_6126_20260912.md` | A |
...
| REQ-UI-024/025/026 (Part card led by its name, Template list selection, export scoped to one PO) | `tests/e2e/router-ui-polish.spec.js` (8 cases), `tests/e2e/part-block-primitive.spec.js`, `tests/test_excel_import_export_source.py` | A |
