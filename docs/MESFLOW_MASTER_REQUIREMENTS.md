...
| REQ-UI-024 | **A Part is named by its Vietnamese name, not by its position in a list.** The shared Part card (Template detail and PO detail) leads with the Part's Vietnamese NAME as the primary, largest text; the Part code and the operation count sit beneath it as one secondary, muted meta line whose items share a common vertical centre. The label `Part 1` / `Part 2` is never rendered — it is an index of the screen, not the name of anything on the shop floor, and it pushed the one string the user is actually scanning for (`Thân thùng rác`) behind a number and a machine code. Both screens use the same markup and the same CSS; they may not drift. |
...
| REQ-UI-024 (Part card led by its name) | `tests/e2e/part-card-identity.spec.js` (4 cases), `tests/e2e/part-block-primitive.spec.js` | A |
