...
| REQ-UI-016 | **Một thay đổi UI chưa được coi là xong nếu chưa có người/agent NHÌN nó.** Quy trình bắt buộc: `functional -> DOM/style contract -> responsive screenshot audit (390/768/1366/1920) -> visual review -> merge`. Bước "visual review" nghĩa là mở từng ảnh chụp ra xem và viết lại thấy gì — lưu file rồi báo số lượng ảnh KHÔNG tính là đã làm. Quy trình đầy đủ và checklist: `skills/visual-ui-audit/SKILL.md`; guard tự động: `tests/e2e/visual-contract.spec.js`. |
| REQ-UI-017 | **Guard thị giác không được im lặng khi dữ liệu thiếu.** Một bài test bỏ qua vì không tìm thấy token/phần tử phải ĐỎ, không được xanh: một bộ dò không bao giờ đỏ trông y hệt một hệ thống sạch. Mọi guard thị giác mới phải kèm negative proof — phá đúng thứ nó canh, chứng minh nó đỏ và nêu đúng tên selector/token, rồi hoàn nguyên. |
