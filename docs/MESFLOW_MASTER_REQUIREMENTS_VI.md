...
| REQ-UI-024 | **Part được gọi bằng TÊN tiếng Việt của nó, không bằng vị trí trong danh sách.** Thẻ Part dùng chung (Template detail và PO detail) mở đầu bằng TÊN tiếng Việt của Part -- chữ chính, to nhất; mã Part và số Operation nằm dưới thành MỘT dòng chú thích phụ, mờ, các thành phần căn giữa theo cùng một đường ngang. Nhãn `Part 1` / `Part 2` không bao giờ được vẽ ra -- đó là số thứ tự của giao diện, không phải tên của thứ gì trong xưởng, và nó đẩy đúng cái chuỗi người dùng đang tìm (`Thân thùng rác`) ra sau một con số và một mã máy. Hai màn dùng CÙNG markup và CÙNG CSS; không được trôi khỏi nhau. |
...
| REQ-UI-024 (thẻ Part lấy tên làm chính) | `tests/e2e/part-card-identity.spec.js` (4 case), `tests/e2e/part-block-primitive.spec.js` | A |
