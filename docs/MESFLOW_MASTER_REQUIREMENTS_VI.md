...
- **Danh tính Operation**: `operation.id` là danh tính kỹ thuật canonical. Còn **duy nhất về mặt nghiệp vụ thì theo phạm vi Part**: `(part_id, mã OP / source_op_no)`.
  - Trùng số OP giữa **hai Part khác nhau** là **hợp lệ** — mỗi tờ bản vẽ đánh số lại từ OP01. Không lỗi, không cảnh báo.
  - Trùng số OP trong **cùng một Part** là **LỖI CHẶN**: một Part không thể có hai công đoạn số 02. Cả lần nhập bị từ chối, **không tạo/cập nhật Template, PO hay Operation nào**. Câu lỗi cố định: `Lỗi: OP trùng mã trong cùng một Part. Vui lòng sửa lại file Excel.`, kèm sheet, Part, mã OP và **tất cả** các dòng đang đụng nhau.
  - **Không** tự sinh hậu tố (`-OP02-2`) để cho qua: làm vậy là hợp thức hoá lỗi và đưa vào sản xuất một mã không có trong tài liệu của khách. Không có đường ghi đè trong luồng thường.
  - Số OP và tiêu đề vẫn **giữ nguyên văn như trên giấy** (`source_op_no`, `source_title`) — chúng là dữ liệu nghiệp vụ/hiển thị, không phải danh tính.
- **Hai loại sheet, cả hai đều hợp lệ**: sheet có khai `TÊN BẢN VẼ` / `MÃ BẢN VẼ` là **DRAWING_PART_SHEET**; sheet không khai cái nào là **PROCESS_LEVEL_SHEET** — một bước quy trình (hàn, sơn, lắp ráp) không thuộc riêng bản vẽ nào. Danh tính bản vẽ là **tuỳ chọn**: sheet cấp quy trình là bình thường và **không bao giờ được cảnh báo** vì thiếu nó, cũng không được hiện cho người dùng mã `PART-xx` tự bịa. Chỉ cảnh báo (`DRAWING_SHEET_MISSING_CODE`) khi sheet **có tên** bản vẽ nhưng **thiếu mã** — lúc đó danh tính mới điền dở, và đó là lỗi thật.
- **Sheet quy trình không có block OPERATION**: không im lặng bỏ qua nữa. Nhập sẽ cảnh báo `Sheet '<tên>' không có block OPERATION; MESFlow sẽ tạo 1 Operation quy trình từ tên sheet.` và tạo **đúng MỘT** Operation quy trình lấy tên từ sheet, **không** có `source_op_no` (trên giấy không có số nào để giữ). Bỏ qua như trước làm cả tờ biến mất khỏi PO, nên bước đó không lên lịch được và cũng không quét được.
- **Mã bản vẽ được LƯU, không chỉ đọc**: `MÃ BẢN VẼ` là danh tính kỹ thuật mà cả xưởng gọi tên chi tiết theo — thợ đọc nó trên bản vẽ giấy, kho dán nó lên thùng, QC ghi nó vào phiếu. Lưu **nguyên văn** vào `template_parts.drawing_code` (khác `code`, vốn đã lọc ký tự để làm mã nội bộ an toàn), và hiện ở Template detail (sửa được) lẫn PO detail (chỉ xem). `parts.drawing_code` là **bản chụp lúc tạo PO**, không đọc xuyên sang Template: Template còn sửa được sau khi PO đã chạy, và sửa Template không được lặng lẽ viết lại tài liệu của một PO đang sản xuất. Bản chụp **không hồi tố** — Part tạo trước migration `0053` giữ `NULL`, và `NULL` nói đúng sự thật là lúc đó hệ thống không biết mã. Tờ mức quy trình không có bản vẽ cũng giữ `NULL`; không bịa ra gì.
...
| Tương phản vùng điều khiển | Kiosk là màn TỐI duy nhất trong một shell SÁNG. Bộ chọn PO, `Làm mới` và `Thoát` dùng **một** bộ token `--k-control-*` của kiosk — không màu viết cứng, không gắn màu vào từng `<option>`. Mọi state (nghỉ/hover/focus/disabled, kể cả `<option>` trong danh sách) phải đạt **≥ 4.5:1** giữa chữ và nền hiệu dụng, ở cả 1920×1080, 1366×768 và mobile. Nền trắng của lớp control chung (`background:#fff!important` cho mọi `select`) phải bị ghi đè có chủ đích — bỏ ghi đè là lỗi chữ-trắng-trên-nền-trắng (đã đo 1.16:1). Popup `<option>` native tối bằng `color-scheme:dark`, không dựa vào kế thừa màu. |
...
| REQ-UI-021 | **Rail điều hướng trái là MỘT mặt tối với MỘT thang trạng thái.** Mọi hàng điều hướng — mục cấp một, nút nhóm, mục con, tiêu đề nhóm con — nằm trên chính nền navy của rail (`--nav-surface`); một mặt sáng hay trắng ở bất kỳ đâu trong rail là defect, kể cả khi nó là một `<button>` rơi về mặt nút mặc định của trình duyệt. Màu điều hướng CHỈ đến từ các token `--nav-*` trong khối `:root` duy nhất — không rule nào chạm hàng điều hướng được mang mã màu viết cứng, và mọi khai báo nền `.app-sidebar` trỏ về cùng một token. Bốn trạng thái phải phân biệt được ngay bằng mắt, và chúng khác nhau về HƯỚNG chứ không chỉ về độ đậm: nhóm đang mở (`--nav-surface-open`) và hàng dưới con trỏ (`--nav-surface-hover`) LÕM xuống dưới mặt rail, còn hàng ứng với trang đang mở NỔI lên (`--nav-surface-active`) và là hàng DUY NHẤT mang thanh accent `inset 3px var(--nav-accent)` — giống hệt nhau ở rail mở rộng và rail thu gọn. Chữ điều hướng giữ tương phản ≥4.5:1 ở mọi trạng thái, kể cả dòng mô tả của mục con; focus bàn phím vẽ vòng riêng bằng `--nav-accent` thay cho vòng chung vốn gần như không thấy trên nền navy; nhóm xổ được phải khai `aria-expanded` đi cùng trạng thái thị giác. |
| REQ-UI-022 | **Danh sách Production Order là DANH SÁCH THẺ, không phải bảng** (bản áp dụng của REQ-UI-017 cho danh sách PO). Mỗi PO trên `/app?page=production-orders` là một `.po-card` riêng: viền đóng đủ bốn cạnh, có nền riêng và shadow nhẹ, bo góc bằng token canonical `--radius-surface` (không bao giờ viết cứng px). Hai thẻ liền nhau cách nhau bằng KHOẢNG CÁCH DỌC THẬT (> 0px) do khung danh sách cấp — một `border-bottom` chạy xuyên suốt KHÔNG thoả điều kiện này. Bên trong thẻ, phân cấp là cố định: mã PO là tiêu đề chính, tên sản phẩm nằm dưới; còn trạng thái, ưu tiên, template nguồn và mốc bắt đầu/kết thúc dự kiến là metadata PHỤ, cỡ chữ nhỏ hơn tiêu đề; khối thao tác gọn (nhiều nhất một primary theo trạng thái, cộng menu thao tác phụ, theo đúng quy tắc row-menu của REQ-UI-018). Mọi biến thể trạng thái đi qua CÙNG một renderer, nên DRAFT/PLANNED/RELEASED/IN_PROGRESS/PAUSED/COMPLETED/CANCELLED đều là thẻ — kể cả những trạng thái không có nút primary. Ở 390px danh sách không được trượt ngang và không được giấu trạng thái/hạn/thao tác sau một thanh cuộn ngang: thẻ xuống một cột và mọi thông tin vẫn đọc được tại chỗ. Yêu cầu này KHÔNG đụng tới business logic — lọc, sắp xếp, phân trang, bấm mở PO, RBAC và deep-link `?po_id=` giữ nguyên hành vi cũ. |
| REQ-UI-023 | **Trên điện thoại, bộ lọc của "Tiến trình sản xuất" là CÔNG CỤ, không phải nội dung** (bản áp dụng của REQ-UI-019 cho màn Gantt). Ở bề rộng <= 700px, `/app?page=production-schedule` chỉ giữ TỐI ĐA MỘT hàng gọn sticky (44-60px) ngay dưới workspace header: nút `Bộ lọc` kèm số bộ lọc đang bật, một dòng tóm tắt tình hình xưởng, và nút cập nhật. Khối lọc chi tiết — các ô lọc, bốn thẻ KPI tóm tắt và chú giải màu — KHÔNG được nằm thường trực trong dòng chảy sticky; nó mở ra thành một tấm nằm NGOÀI dòng chảy (`position:fixed`), không bao giờ che kín viewport (vẫn thấy một dải danh sách bên dưới), có nút đóng rõ ràng, tự cuộn bên trong với `overscroll-behavior:contain`, và tự đóng sau khi người dùng đã chọn xong một giá trị lọc (gõ vào ô tìm nhanh thì KHÔNG đóng). Offset sticky cấp cho header từng nhóm PO (`--schedule-toolbar-height`) phải bám đúng chiều cao THẬT của hàng sticky đó, để mở tấm lọc không bao giờ đẩy header PO ra ngoài màn hình. Sau khi đóng tấm lọc, phần màn hình còn lại dưới cụm chrome sticky cấp trang (workspace header + thanh lọc) phải còn ít nhất 70% chiều cao viewport cho chính Gantt. Vuốt dọc, lăn chuột hoặc kéo con trỏ BẮT ĐẦU trên thanh công cụ, trên bộ lọc, hoặc bên trong chart đều phải cuộn trang bình thường — không `touch-action`, `overflow` hay listener `preventDefault` nào được nuốt cử chỉ đó — trong khi vuốt ngang trong chart vẫn kéo được timeline, và chart không được sinh thêm một vùng cuộn DỌC lồng nhau. Trang không được tràn ngang ở 390px, 375px và 320px. Desktop (>700px) giữ nguyên: khối lọc đầy đủ vẫn nằm trong dòng chảy và hàng gọn không được vẽ ra. Yêu cầu này KHÔNG đụng tới business logic — lọc, tìm nhanh, mở/thu gọn, tự làm mới, trạng thái URL `?po=/?q=/?opstate=` và RBAC giữ nguyên hành vi cũ. |
| REQ-UI-024 | **Part được gọi bằng TÊN tiếng Việt của nó, không bằng vị trí trong danh sách.** Thẻ Part dùng chung (Template detail và PO detail) mở đầu bằng TÊN tiếng Việt của Part -- chữ chính, to nhất; mã Part, mã bản vẽ và số Operation nằm dưới thành MỘT dòng chú thích phụ, mờ, các thành phần căn giữa theo cùng một đường ngang. Nhãn `Part 1` / `Part 2` không bao giờ được vẽ ra -- đó là số thứ tự của giao diện, không phải tên của thứ gì trong xưởng, và nó đẩy đúng cái chuỗi người dùng đang tìm (`Chân ghế A  - Trái`) ra sau một con số và một mã máy. Hai màn dùng CÙNG markup và CÙNG CSS; không được trôi khỏi nhau. |
| REQ-UI-025 | **Đang chọn cái gì thì phải nhìn thấy, focus được và đọc ra được.** Danh sách Template là những thẻ RỜI -- mỗi thẻ có viền và bo góc riêng, cách nhau bằng `gap` dọc lấy từ thang khoảng cách, không phải một khối liền chia bằng đường kẻ. Thẻ đang chọn mang `aria-current="true"` (thuộc tính mà cả CSS trạng thái chọn lẫn bài test đều bám vào; `aria-selected` không hợp lệ trên role `button` và bị trợ năng bỏ qua), và nói ra trạng thái đó bằng nhiều hơn một kênh: nền khác, viền đậm hơn, và một vạch màu. Tiêu điểm bàn phím vẽ viền `:focus-visible` nhìn thấy được. Một thẻ trong danh sách KHÔNG phải là surface thẻ theo nghĩa của rule quét `[class$="-card"]` -- rule đó đặt `background:...!important` và đã âm thầm đè mọi trạng thái chọn, chính là lỗi mà yêu cầu này đóng lại. |
| REQ-UI-026 | **Xuất file thuộc về bản ghi được xuất.** Màn DANH SÁCH Production Order không có nút xuất: nút cũ tải `/api/operations/export.xlsx` -- toàn bộ Operation của MỌI PO, tức tài liệu của không ai cả. Xuất Excel bắt đầu từ màn chi tiết của MỘT PO (`Xuất Excel + QR`, REQ-TPL-006). |
...
| REQ-TPL-007 (nhập Router: đọc hết field, tạo cả PO) | `tests/integration/test_part_drawing_code.py` (MÃ BẢN VẼ lưu nguyên văn, chụp lúc tạo PO, không hồi tố, không mất khi lưu lại cây Template), `tests/integration/test_migration_0053_part_drawing_code.py`, `tests/integration/test_template_op_code_scoped_by_part.py` (trùng số OP trong cùng Part -> lỗi chặn kèm sheet/Part/mã/mọi dòng; khác Part -> hợp lệ), `tests/test_router_import_field_semantics.py` (36 case: đơn vị s/phút/giờ + biến thể, ví dụ 60 phút/180 s/6,5 h, 0 vs `-` vs trống, tổng thời gian giữ nguyên khi lệch, số lượng Part độc lập QTY), `tests/test_router_po_import_flow.py`, `tests/integration/test_template_op_code_scoped_by_part.py` (trùng trong cùng Part -> lỗi chặn nêu sheet/Part/mã/mọi dòng; trùng giữa hai Part -> hợp lệ), `tests/integration/test_migration_0052_router_source_semantics.py`, `reports/ROUTER_IMPORT_FIELD_AUDIT_6126_20260912.md` | A |
...
| REQ-UI-021 (mặt nav + thang trạng thái) | `tests/e2e/sidebar-nav-surface.spec.js`, `test_sidebar_nav_single_palette.py` | A |
| REQ-UI-022 (tách thẻ danh sách PO) | `tests/e2e/po-list-card-contract.spec.js`, `tests/e2e/po-action-menu.spec.js` | A |
| REQ-UI-024/025/026 (thẻ Part lấy tên làm chính, trạng thái chọn ở danh sách Template, xuất Excel theo PO) | `tests/e2e/router-ui-polish.spec.js` (8 case), `tests/e2e/part-block-primitive.spec.js`, `tests/test_excel_import_export_source.py` | A |
| REQ-UI-023 (thanh lọc Gantt trên điện thoại) | `tests/e2e/schedule-mobile-scroll-contract.spec.js`, `tests/e2e/schedule-mobile-layout.spec.js`, `tests/e2e/production-progressive-disclosure.spec.js` | A |
